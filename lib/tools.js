...
export function toolCatalogue(extraTools = {}) {
  const all = { ...TOOLS, ...extraTools };
  return Object.entries(all).map(([name, t]) => ({ name, description: t.description }));
}

/** Execute a single named tool with arguments. extraTools (e.g. MCP) take precedence. */
export async function executeTool(name, args, ctx = {}, extraTools = {}) {
  const tool = extraTools[name] || TOOLS[name];
 ...
 */
export async function runAgentLoop({ provider, apiKey, model, query, temperature = 0.5, maxTokens = 2048, maxSteps = 6, signal, onEvent, extraTools = {}, baseURL = null }) {
  const catalogue = toolCatalogue(extraTools);
  const messages = [
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    const res = await callAI(provider, apiKey, model, messages, { temperature, maxTokens, signal, baseURL });
    ...
    if (obj && obj.tool) {
      ...
      let observation;
      try {
        const result = await executeTool(obj.tool, obj.args, { signal }, extraTools);
