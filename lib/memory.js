...
export class MemoryStore {
  constructor({ persistDir = null } = {}) {
    ...
    this.shares = new Map();
    /** @type {Map<string, object>} personas / custom assistants */
  ...
  createPersona({ name = 'Assistant', systemPrompt = '', provider = null, model = null, temperature = null, tools = false } = {}) {
    const now = Date.now();
    const persona = { id: id('persona'), name: String(name).slice(0, 80), systemPrompt: String(systemPrompt), provider, model, temperature, tools: !!tools, createdAt: now, updatedAt: now };
  ...
  listPersonas() {
    return [...this.personas.values()].sort((a, b) => b.updatedAt - a.updatedAt);
  }

  getPersona(pid) {
    return this.personas.get(pid) || null;
  }

  updatePersona(pid, patch) {
    const persona = this.personas.get(pid);
    if (!persona) throw Object.assign(new Error('Persona not found'), { status: 404 });
    const p = patch || {};
    // Coerce to the same types createPersona() enforces.
    if ('name' in p) persona.name = String(p.name).slice(0, 80);
    if ('systemPrompt' in p) persona.systemPrompt = String(p.systemPrompt);
    if ('provider' in p) persona.provider = p.provider;
    if ('model' in p) persona.model = p.model;
    if ('temperature' in p) persona.temperature = (p.temperature === null || typeof p.temperature === 'number') ? p.temperature : Number(p.temperature) || null;
    if ('tools' in p) persona.tools = !!p.tools;
  ...
  deletePersona(pid) {
    const existed = this.personas.delete(pid);
    this._persist();
    return existed;
  }
