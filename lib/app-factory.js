...
import { callAI, streamAI, listModels, providerMeta, autoRoute } from './providers.js';
...
export function createApp(cfg = {}) {
  ...
  app.get('/api/tools', (_req, res) => res.json({ success: true, tools: toolCatalogue() }));
  ...
  app.post('/api/analyze', async (req, res) => {
    try {
      ...
      ], { temperature: clampNumber(temperature, 0, 2, 0.7), maxTokens: clampNumber(maxTokens, 1, 128000, 2048) });
  ...
  app.post('/api/rag/chat', async (req, res) => {
    try {
      ...
      const { context, hits } = rag.buildContext(lastUser?.content || '', clampNumber(req.body.k, 1, 10, 4));
      const sys = context
  ...
  return { app, activeRuns, rag, memory };
}
