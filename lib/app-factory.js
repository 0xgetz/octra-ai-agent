...
import { scanText } from './guard.js';
import { Metrics, metricsMiddleware } from './observability.js';
import { runNativeAgent, buildToolSpecs } from './native-tools.js';
...
export function createApp(cfg = {}) {
  ...
  // ---- Observability + response cache (created early so middleware can time everything) ----
  ...
  app.get('/api/tools/native', (_req, res) => res.json({ success: true, tools: buildToolSpecs(mcp.toolMap()) }));
  ...
  // ===================== Insights: cost, metrics, cache, guardrails =====================
  app.post('/api/cost', (req, res) => {
    const { provider, model, usage } = req.body || {};
    if (!provider || !model) return res.status(400).json({ success: false, error: 'Missing provider or model' });
    return res.json({ success: true, cost: estimateCost(provider, model, usage || {}) });
  });
  ...
  app.get('/api/metrics', (req, res) => {
    ...
    return res.json({ success: true, metrics: metrics.snapshot(), cache: cache.stats() });
  });

  app.get('/api/cache', (_req, res) => res.json({ success: true, cache: cache.stats() }));
  app.delete('/api/cache', (_req, res) => res.json({ success: true, cleared: cache.clear() }));

  app.post('/api/guard/scan', (req, res) => {
    const { text } = req.body || {};
    if (typeof text !== 'string') return res.status(400).json({ success: false, error: 'Missing text (string)' });
    return res.json({ success: true, ...scanText(text) });
  });
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      const temp = clampNumber(temperature, 0, 2, 0.7);
      const allMessages = systemPrompt ? [{ role: 'system', content: systemPrompt }, ...messages] : messages;
      const cacheParams = { provider, model, messages: allMessages, temperature: temp, systemPrompt: systemPrompt || null };

      // Serve identical requests from the response cache (opt out with cache:false).
      if (req.body.cache !== false) {
        const cached = cache.get(cacheParams);
        if (cached.hit) {
          return res.json({ success: true, message: cached.value.message, usage: cached.value.usage, cost: cached.value.cost, cached: true, cacheType: cached.type });
        }
      }
      ...
      const cost = estimateCost(provider, model, result.usage);
      metrics.addUsage(result.usage, cost.totalCost);
      if (req.body.cache !== false) cache.set(cacheParams, { message: result.message, usage: result.usage, cost });
      return res.json({ success: true, message: result.message, usage: result.usage, cost });
    } catch (err) {
  ...
  // ===================== Agentic single-query (JSON ReAct tools) =====================
  ...
  // ===================== Agentic with native function-calling + parallel tools =====================
  app.post('/api/agent/native', async (req, res) => {
    const { provider, apiKey, model } = req.body || {};
    const query = req.body?.query;
    const messages = req.body?.messages;
    if (!provider || !apiKey || !model || (!query && !Array.isArray(messages))) {
      return res.status(400).json({ success: false, error: 'Missing required fields: provider, apiKey, model, and query or messages' });
    }
    const send = openSSE(res);
    const controller = new AbortController();
    req.on('close', () => controller.abort());
    try {
      const result = await runNativeAgent({
        provider, apiKey, model, query, messages,
      ...
      const cost = estimateCost(provider, model, result.usage);
      metrics.addUsage(result.usage, cost.totalCost);
      send('done', { answer: result.answer, steps: result.steps, usage: result.usage, cost });
    } catch (err) {
      if (err.name !== 'AbortError') send('error', { error: err.message });
    }
    return res.end();
  ...
  return { app, activeRuns, rag, memory, mcp, metrics, cache };
}
