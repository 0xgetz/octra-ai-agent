...
function buildRequest(provider, apiKey, model, messages, { temperature, maxTokens, stream, tools }) {
  const p = PROVIDERS[provider];
  if (p.kind === 'openai') {
    const body = { model, messages, temperature, max_tokens: maxTokens, stream: !!stream };
  ...
  if (p.kind === 'claude') {
    ...
    const body = { model, messages: rest, max_tokens: maxTokens || 2048, temperature, stream: !!stream };
  ...
  if (p.kind === 'gemini') {
    ...
    return {
      url: `${p.base}/models/${model}:${verb}${sep}${encodeURIComponent(apiKey)}`,
...
export async function callAI(provider, apiKey, model, messages, opts = {}) {
  ...
  const { temperature = 0.7, maxTokens = 2048, signal = null, tools = null } = opts;
  const fetchSignal = signal || AbortSignal.timeout(opts.timeoutMs || DEFAULT_TIMEOUT_MS);

  const { url, headers, body } = buildRequest(provider, apiKey, model, messages, { temperature, maxTokens, stream: false, tools });
...
export async function* streamAI(provider, apiKey, model, messages, opts = {}) {
  if (!isValidProvider(provider)) throw new Error(`Unsupported provider: ${provider}`);
  const { temperature = 0.7, maxTokens = 2048, signal = null } = opts;
  const fetchSignal = signal || AbortSignal.timeout(opts.timeoutMs || DEFAULT_TIMEOUT_MS);

  const { url, headers, body } = buildRequest(provider, apiKey, model, messages, { temperature, maxTokens, stream: true });
  const response = await fetch(url, { method: 'POST', headers, body: JSON.stringify(body), signal: fetchSignal });
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    const msg = err.error?.message || err.error?.[0]?.message || err.message || response.statusText;
